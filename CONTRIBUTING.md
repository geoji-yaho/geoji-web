...
| `src/shared/api/api-error.ts`              | `ApiError`와 `isApiError`. `kind`는 badRequest, unauthorized, forbidden, notFound, conflict, server, network, timeout, unknown 9종                                                                                                                |
...
| `src/shared/api/posts.ts`                  | 게시물 엔티티. 제출 `submitPost`와 `completeSubmission`, 투표 `castPostVote`, 댓글 `createPostComment`와 `removePostComment`, 게시물 삭제 `removePost`. 조회 팩토리 `postQueries`는 `detail`, `feed`, `comments`, `verdict`, `shareCard` 다섯이다 |
...
| `src/shared/hooks/useCreatePostComment.ts` | 댓글 작성 변이. 성공하면 그 게시물의 댓글 목록을 무효화한다. 방 피드와 판결 결과가 같이 쓴다                                                                                                                                                      |
| `src/shared/hooks/useRemovePostComment.ts` | 댓글 삭제 변이. 같은 댓글 목록을 무효화한다. 두 화면이 같이 쓴다                                                                                                                                                                                  |
| `src/shared/hooks/useRouteNotice.ts`       | 화면 사이 알림. `navigate`가 넘긴 `state.notice`를 읽어 토스트 문구로 준다. 읽은 직후 히스토리 state를 비우고 잠시 뒤 `null`이 된다                                                                                                               |
| `src/shared/hooks/useMyMonthStats.ts`      | 프로필과 방 목록, 방별 게시물 피드를 합쳐 이번 달 지출액과 거지력, 티어, 판결 이력을 계산한다. 홈과 마이페이지가 같이 쓴다                                                                                                                        |
| `src/shared/lib/platform.ts`               | `shareToKakao`와 `loadKakaoSdk`, `shareContent`, `copyText`, `downloadDataUrl`. `navigator.share`가 없거나 거부되면 링크를 클립보드에 복사하고 결과만 돌려준다                                                                                    |
| `src/shared/components/CommentSheet.tsx`   | 댓글 바텀시트. 목록과 입력, 본인 댓글 삭제를 그리고 방 피드와 판결 결과가 같이 쓴다                                                                                                                                                               |
