...
# 광범위 스테이징, .env 파일 스테이징, 훅을 건너뛰는 커밋과 푸시, 강제 푸시, main 에 직접 푸시,
# main 에서의 직접 커밋이다.
...
add_args="$(printf '%s' "$command" | grep -oE 'git[[:space:]]+add[[:space:]]+[^|;&]*')"
if [ -n "$add_args" ] && printf '%s' "$add_args" | grep -oE '(^|[[:space:]])(\./)?\.env(\.[^[:space:]]*)?' | grep -vqE '\.env\.example$'; then
	deny ".env 파일은 스테이징하지 않는다. 값은 .env.example 에 자리만 남긴다 (git-workflow.md 금지 패턴 3)"
fi
...
if printf '%s' "$command" | grep -qE 'git[[:space:]]+push[[:space:]].*[[:space:]](origin[[:space:]]+)?main([[:space:]]|$)'; then
	deny "main 에 직접 푸시하지 않는다. main 에 들어가는 길은 PR 하나다 (git-workflow.md 금지 패턴 1)"
fi
...
if printf '%s' "$command" | grep -qE 'git[[:space:]]+commit([[:space:]]|$)'; then
	...
	case "$branch" in
		main) deny "main 에서 직접 커밋하지 않는다. feature/ 브랜치를 딴다 (git-workflow.md 금지 패턴 1)" ;;
	esac
fi
