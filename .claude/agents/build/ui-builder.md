...
`folder-structure.md`와 `tailwind.md`는 세션에 실려 있다. 화면 배치와 상태는 `docs/design/DESIGN-SPEC.md`의 해당 화면 절, 토큰의 용도와 카피 톤은 `docs/design/DESIGN.md`, 이미 있는 부품은 `src/shared/ui/`와 `src/shared/components/`다. 같은 모양을 새로 만들기 전에 있는 것을 먼저 찾는다.
